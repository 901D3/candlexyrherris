...
(function () {
  ...
  function renderAsWebP(canvas, quality) {
    let frame =
      typeof canvas === "string" && /^data:image\/webp/.test(canvas) ? canvas : canvas.toDataURL("image/webp", quality);
  ...
  function extractKeyframeFromWebP(webP) {
    ...
    while (cursor < webP.length - 8) {
      let fourCC = String.fromCharCode(webP[cursor], webP[cursor + 1], webP[cursor + 2], webP[cursor + 3]);
      cursor += 4;

      let chunkLength = webP[cursor] | (webP[cursor + 1] << 8) | (webP[cursor + 2] << 16) | (webP[cursor + 3] << 24);
      cursor += 4;

      switch (fourCC) {
        case "VP8 ":
          return {
            frame: webP.subarray(cursor, cursor + chunkLength),
            hasAlpha: hasAlpha,
          };
    ...
    return function (options) {
      ...
      function writeHeader() {
        let ebmlHeader = {
            ...
            data: [
              ...
              {id: 0x4282, data: strToU8("webm")}, // DocType
          ...
          segmentInfo = {
            ...
            data: [
              ...
              {id: 0x4d80, data: strToU8("webm-writer-js")}, // MuxingApp
              {id: 0x5741, data: strToU8("webm-writer-js")}, // WritingApp
              segmentDuration, // will be patched later
            ],
          },
          videoProperties = [
        ...
        if (options.transparent) {
          videoProperties.push({id: 0x53c0, data: 1}); // AlphaMode
        }
          ...
          data: [
            {
              id: 0xae, // TrackEntry
              data: [
                {id: 0xd7, data: DEFAULT_TRACK_NUMBER}, // TrackNumber
                {id: 0x73c5, data: DEFAULT_TRACK_NUMBER}, // TrackUID
                {id: 0x9c, data: 0}, // FlagLacing
                {id: 0x22b59c, data: strToU8("und")}, // Language
                {id: 0x86, data: strToU8("V_VP8")}, // CodecID
                {id: 0x258688, data: strToU8("VP8")}, // CodecName
                {id: 0x83, data: 1}, // TrackType = video
                {id: 0xe0, data: videoProperties}, // Video
              ],
            },
          ],
        };
        ...
        ebmlSegment = {
          ...
          data: [segmentInfo, tracks], // no seekhead
        };
      ...
      function flushClusterFrameBuffer() {
        ...
        for (let i = 0; i < clusterFrameBuffer.length; i++) {
          rawImageSize +=
            clusterFrameBuffer[i].frame.length + (clusterFrameBuffer[i].alpha ? clusterFrameBuffer[i].alpha.length : 0);
        }
