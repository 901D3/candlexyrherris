...
var FLACDecoder = AV.Decoder.extend(function () {
  ...
  this.prototype.readChunk = function () {
    ...
    if (channels !== this.format.channelsPerFrame) throw new Error("Switching channel layout mid-stream not supported.");

    // bits per sample
    if (bpsCode === 3 || bpsCode === 7) throw new Error("Invalid sample size code");

    this.bps = SAMPLE_SIZES[bpsCode];
    if (this.bps !== this.format.bitsPerChannel) throw new Error("Switching bits per sample mid-stream not supported.");
  ...
  this.prototype.decode_residuals = function (channel, predictor_order) {
    ...
    if (predictor_order > samples) throw new Error("Invalid predictor order " + predictor_order + " > " + samples);
